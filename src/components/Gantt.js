...
const Gantt = ({ ganttData, selectedCommesse, projectResources, onUpdateEvent, onSaveEvent, onDeleteEvent, allCollaborators }) => {
  ...
  const structuredData = selectedCommesse.flatMap((commessaId) => {
    ...
    return commessaEvents.map((event) => {
      const resources = (event.IncaricatoImages || []).map((img, index) => ({
        image: img,
      ...
      return {
        ...event,
        CommessaName: commessa?.text || `Commessa ${commessaId}`,
        CategoryColor: commessa?.color || '#000000',
        resources, // Aggiungi qui le risorse per ogni evento
      };
    });
  });
    ...
    <div>
      {structuredData.length > 0 ? (
                ...
                if (!data.taskData.IncaricatoImages || data.taskData.IncaricatoImages.length === 0) {
                  return (
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '5px' }}>
                      ...
                      />
                      <span>Nessun collaboratore</span>
                    </div>
                  );
                }
                ...
                return (
                  <div>
                    {data.taskData.IncaricatoImages.map((img, index) => {
                      const names = data.taskData.IncaricatoName.split(", ");
                      const name = names[index] || "Collaboratore sconosciuto";
                      return (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '5px' }}>
                          <img
                            src={img}
                            alt={name}
                            style={{ width: '25px', height: '25px', borderRadius: '20%', marginRight: '4px' }}
                          />
                          <span>{name}</span>
                        </div>
                      );
        ...
        >
          ...
          <Inject services={[Selection, DayMarkers, Toolbar, Edit, Resize, RowDD, Filter, DayMarkers]} />
