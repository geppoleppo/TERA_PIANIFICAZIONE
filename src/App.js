...
const App = () => {
    ...
    const fetchData = async () => {
      try {
        const [collaboratoriResponse, commesseResponse, mysqlCommesseResponse, selectedCommesseResponse] = await Promise.all([
          axios.get('api/collaboratori'),
          axios.get('api/commesse'),
          axios.get('api/commesse-mysql'),
          axios.get('api/commesse')
        ]);
        ...
        const eventiResponse = await axios.get('http://api/eventi');
  ...
  const saveSelectedCommesse = async () => {
    try {
      ...
      await axios.post('http://api/update-sqlite', { commesse: commesseToSave });
      // Fetch updated data and update state
      const updatedCommesseResponse = await axios.get('http://api/commesse');
  ...
  const handleSchedulerDataChange = debounce((args) => {
    ...
    switch (args.requestType) {
      case 'eventCreated':
        axios.post('http://api/eventi', event)
          .then(response => {
        ...
        break;
      case 'eventChanged':
        axios.put(`http://api/eventi/${event.Id}`, event)
          .then(() => {
        ...
        break;
      case 'eventRemoved':
        axios.delete(`http://api/eventi/${event.Id}`)
          .then(() => {
  ...
  const handleGanttDataChange = debounce((args) => {
    ...
    switch (args.requestType) {
      case 'eventChanged':
        axios.put(`http://api/eventi/${task.Id}`, task)
          .then(() => {
        ...
        break;
      case 'eventRemoved':
        axios.delete(`http://api/eventi/${task.Id}`)
          .then(() => {
