...
const App = () => {
    ...
    const fetchData = async () => {
      try {
        const [
          ...
          axios.get('http://localhost:4443/api/collaboratori'),
          axios.get('http://localhost:4443/api/commesse'),
          axios.get('http://localhost:4443/api/commesse-mysql'),
          axios.get('http://localhost:4443/api/commesse')
        ]);
  ...
  const handleCollaboratoreChange = async (option) => {
    ...
    try {
      const response = await axios.get(`http://localhost:4443/api/commesse/collaboratore/${option.value}`);
  ...
  const saveSelectedCommesse = async () => {
    ...
    try {
      ...
      const updatedCommesseResponse = await axios.get(`http://localhost:4443/api/commesse/collaboratore/${selectedCollaboratore.value}`);
  ...
  const handleSchedulerDataChange = debounce((args) => {
    ...
    switch (args.requestType) {
      case 'eventCreated':
        ...
        break;
      case 'eventChanged':
        axios.put(`http://localhost:4443/api/eventi/${event.Id}`, event)
          .then(() => {
        ...
        break;
      case 'eventRemoved':
        axios.delete(`http://localhost:4443/api/eventi/${event.Id}`)
          .then(() => {
  ...
  const handleGanttDataChange = debounce((args) => {
    ...
    switch (args.requestType) {
      case 'eventChanged':
        axios.put(`http://localhost:4443/api/eventi/${task.Id}`, task)
          .then(() => {
        ...
        break;
      case 'eventRemoved':
        axios.delete(`http://localhost:4443/api/eventi/${task.Id}`)
          .then(() => {
